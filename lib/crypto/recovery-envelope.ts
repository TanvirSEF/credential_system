...
export async function unlockVaultWithRecoveryKey(
  ...
  try {
    ...
    const recoveryKek = await deriveKeyFromPassword(normalizedKey, saltBytes, iterations);
    const vaultKey = await unwrapVaultKey(recoveryEnvelope.wrappedKey, recoveryEnvelope.iv, recoveryKek);
