    ...
    <aside className="fixed right-4 bottom-24 z-50 w-[calc(100%-2rem)] max-w-sm rounded-2xl border bg-card/95 p-4 shadow-2xl shadow-black/20 backdrop-blur-xl md:bottom-5">
      ...
      <div className="flex items-start gap-3 pr-7">
        <div className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary">
          {updateAvailable ? <RefreshCw className="size-5" /> : <Smartphone className="size-5" />}
        </div>
        <div className="min-w-0">
          <p className="text-sm font-bold">
            {updateAvailable ? "App update ready" : "Install Secure Personal Vault"}
          </p>
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="flex items-start gap-2.5 rounded-xl border bg-muted/20 p-3.5 text-xs leading-relaxed text-muted-foreground">
          ...
          Only public interface assets are cached. Sessions, credentials, documents, and decrypted vault data are never added to the offline cache.
        </div>
        ...
        {updateAvailable ? (
          ...
          <p className="text-xs leading-relaxed text-muted-foreground">
            On Android Chrome, open the browser menu and choose <strong className="text-foreground">Install app</strong> if the install prompt is not currently visible.
          </p>
        )}
      </CardContent>
    </Card>
